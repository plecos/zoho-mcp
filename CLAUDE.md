...
- `tools/mail.py`, `tools/calendar.py`, `tools/contacts.py` — thin MCP tool wrappers that call into the relevant client and shape output for the LLM. No HTTP calls, no token logic — inject the client rather than constructing it inline.
