...
- **Nothing is relayed through a third party.** In its default deployment — the `zoho-mcp` entry point, which is what the MCPB bundle and every documented client config use — the server runs locally over stdio: no listening socket, no hosted endpoint. Outbound calls go to Zoho's own REST APIs; mail bodies and tool arguments don't traverse any intermediary. Worth knowing if you're weighing this against a hosted MCP option (see [docs/vs-zoho-mcp.md](docs/vs-zoho-mcp.md)).
  ...
  **The `zoho-mcp-http` entry point is the exception, and it is opt-in in the strongest sense: it is a separate command you have to choose to run.** It serves the same tools over streamable HTTP so that clients which cannot spawn a local process — phones, principally — can reach them, and that means it does have a listening socket. Two things guard it. It refuses to start unless `ZOHO_HTTP_AUTH_TOKEN` is set, so there is no configuration in which it is reachable without a shared secret; and it binds `127.0.0.1` unless `ZOHO_HTTP_HOST` says otherwise, so exposing it beyond the host is a second deliberate act. Every HTTP request is checked in ASGI middleware ahead of the MCP app, compared with `hmac.compare_digest`, and a request without the secret reaches no tool at all.

  What that gate is **not** is the OAuth flow an MCP client performs against a remote server, and it is not multi-tenancy. It is a single shared secret for a single-user server whose operator and user are the same person. Treat the secret as equivalent to the mailbox: anyone holding it can read and write everything the granted scopes cover. Run it behind TLS — a tunnel or a reverse proxy — never as plain HTTP across a network you don't control, since the secret is a bearer credential and travels in a header.
