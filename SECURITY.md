...
- **Nothing is relayed through a third party.** In its default deployment — the `zoho-mcp` entry point, which is what the MCPB bundle and every documented client config use — the server runs locally over stdio: no listening socket, no hosted endpoint. Outbound calls go to Zoho's own REST APIs; mail bodies and tool arguments don't traverse any intermediary. Worth knowing if you're weighing this against a hosted MCP option (see [docs/vs-zoho-mcp.md](docs/vs-zoho-mcp.md)).
  ...
  **OAuth mode** (`ZOHO_HTTP_AUTH_MODE=oauth`) runs a self-contained OAuth 2.1 authorization server — built on Authlib and joserfc, not hand-rolled — for clients that authenticate only through the OAuth handshake, such as Claude's phone connector. It is still single-user and not multi-tenancy: there is one operator, who approves each connection by entering `ZOHO_OAUTH_OPERATOR_PASSWORD` on the consent page (compared constant-time), and the server issues short-lived access tokens and rotating refresh tokens signed with an auto-generated key. A replayed, rotated-out refresh token is refused and revokes the chain. The signing key, registered clients and refresh ledger are `0600` files under `ZOHO_OAUTH_STATE_DIR`; the access-token gate is the same ASGI chokepoint, verifying the JWT (issuer, audience, expiry, HS256 pinned) before any request reaches a tool. It won't start without `ZOHO_OAUTH_ISSUER` (which must be https) and the operator passphrase — neither has a safe default.
  ...
  Either mode: run it behind TLS — a tunnel or a reverse proxy — never as plain HTTP across a network you don't control, since the bearer secret (or, in OAuth mode, the issued tokens) travels in a header. What has **not** been exercised is Claude's real mobile connector against a deployed instance; the flow is verified end to end against a live local server, but that last mile is unproven until you run it.
