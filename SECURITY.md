...
- **Nothing is relayed through a third party.** In its default deployment — the `zoho-mcp` entry point, which is what the MCPB bundle and every documented client config use — the server runs locally over stdio: no listening socket, no hosted endpoint. Outbound calls go to Zoho's own REST APIs; mail bodies and tool arguments don't traverse any intermediary. Worth knowing if you're weighing this against a hosted MCP option (see [docs/vs-zoho-mcp.md](docs/vs-zoho-mcp.md)).

  **The `zoho-mcp-http` entry point is the exception, and it is opt-in in the strongest sense: it is a separate command you have to choose to run.** It serves the same tools over streamable HTTP so that clients which cannot spawn a local process — phones, principally — can reach them, and that means it does have a listening socket. It binds `127.0.0.1` unless `ZOHO_HTTP_HOST` says otherwise, so exposing it beyond the host is a deliberate act, and every request to the MCP path is checked in ASGI middleware ahead of the MCP app — a request that fails the check reaches no tool at all. It has two authentication modes, and refuses to start if the one you chose isn't fully configured.

  **Bearer mode** (`ZOHO_HTTP_AUTH_MODE=bearer`, the default) gates on a single shared secret: it won't start unless `ZOHO_HTTP_AUTH_TOKEN` is set, and every request's `Authorization: Bearer` is compared with `hmac.compare_digest`. Treat that secret as equivalent to the mailbox — anyone holding it can read and write everything the granted scopes cover. It suits a single-user server reached over the API's `authorization_token` or from behind a private tunnel that does its own auth.

  **OAuth mode** (`ZOHO_HTTP_AUTH_MODE=oauth`) runs a self-contained OAuth 2.1 authorization server — built on Authlib and joserfc, not hand-rolled — for clients that authenticate only through the OAuth handshake, such as Claude's phone connector. It is still single-user and not multi-tenancy: there is one operator, who approves each connection by entering `ZOHO_OAUTH_OPERATOR_PASSWORD` on the consent page (compared constant-time), and the server issues short-lived access tokens and rotating refresh tokens signed with an auto-generated key. A replayed, rotated-out refresh token is refused and revokes the chain. The signing key, registered clients and refresh ledger are `0600` files under `ZOHO_OAUTH_STATE_DIR`; the access-token gate is the same ASGI chokepoint, verifying the JWT (issuer, audience, expiry, HS256 pinned) before any request reaches a tool. It won't start without `ZOHO_OAUTH_ISSUER` (which must be https) and the operator passphrase — neither has a safe default.

  That state directory holds the signing key and any client secrets, so it is as sensitive as the mailbox: keep it private. On a host with ephemeral disk it must be pointed at durable storage — otherwise a restart regenerates the signing key, invalidating every issued token. The [hosting guide](docs/hosting.md) backs it with a private GCS bucket scoped to the runtime service account; whatever the backing, it must not be world- or group-readable.

  Either mode: run it behind TLS — a tunnel or a reverse proxy — never as plain HTTP across a network you don't control, since the bearer secret (or, in OAuth mode, the issued tokens) travels in a header. The OAuth flow has been exercised end to end against Claude's real connector on a live deployment; still, hosts differ, so treat a connect on your own deployment as the thing that confirms it, per [docs/hosting.md](docs/hosting.md).
