...
- `zoho/client.py` — HTTP calls to Zoho Mail/Calendar/Tasks/Notes APIs and raw-JSON-to-normalized-shape conversion. No MCP/tool concepts here. Tasks (`mail.zoho.com/api/tasks`) and Notes (`mail.zoho.com/api/notes`) live here rather than their own modules, unlike Contacts: both share Mail's base domain and OAuth scope family (`ZohoMail.tasks.READ`/`ZohoMail.notes.READ`, alongside `ZohoMail.messages.READ` etc.) rather than being genuinely separate products/scope families.
...
- `tools/mail.py`, `tools/calendar.py`, `tools/tasks.py`, `tools/notes.py`, `tools/contacts.py` — thin MCP tool wrappers that call into the relevant client and shape output for the LLM. No HTTP calls, no token logic — inject the client rather than constructing it inline.
