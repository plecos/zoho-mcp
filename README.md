...
1. Register a Server-based Application in the [Zoho API Console](https://accounts.zoho.com/developerconsole) with scopes `ZohoMail.messages.READ`, `ZohoMail.accounts.READ`, `ZohoMail.folders.READ`, `ZohoCalendar.event.READ`, `ZohoCalendar.calendar.READ`, `zohocontacts.contactapi.READ`, `ZohoMail.tasks.READ`, `ZohoMail.notes.READ`, and `ZohoMail.links.READ` (`.accounts.READ` and `.folders.READ` are also used at runtime, for `search_emails`'s timezone lookup and Sent/Drafts/Templates filtering respectively, not just at setup; `.calendar.READ` is only needed once, to look up the calendar UID in step 4), redirect URI `http://localhost:8765/callback` (change the port via `ZOHO_OAUTH_CALLBACK_PORT` if 8765 is taken).
