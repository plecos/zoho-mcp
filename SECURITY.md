...
- **`ZOHO_ALLOW_AUTO_SEND` is not exposed as a bundle setting.** It stays an environment variable an operator sets deliberately. A labelled toggle in a settings pane would be a materially smaller act than editing a server's environment, and the friction is the protection. Pinned by a test in `tests/test_mcpb_manifest.py`.
