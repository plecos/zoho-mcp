...
`priority` is one of `highest`/`high`/`normal`/`low`/`lowest`, converted from Zoho's numeric field — [verified against the `Importance` header](docs/zoho-api-notes.md#priority-is-x-priority-confirmed-against-the-importance-header) on the same messages. `flag` is Zoho's flag *type* name (`important` is the one observed) and is empty when a message isn't flagged. Zoho sends 21 fields; the six still omitted are omitted for [stated reasons](docs/zoho-api-notes.md#the-thread-fields-still-could-not-be-verified) — `sentDateInGMT` is wrong, the thread fields can't be read from any available data, and the rest are undocumented with no evident use.
